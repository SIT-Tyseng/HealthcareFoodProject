...
print(f"Loaded {id_to_name} categories from {category_file}")
...
print(f"Collected {len(dataset)} samples from {root_dir}")
...
with open(os.path.join(output_dir,'sgfood_sharegpt_train.json'), 'w') as f:
    json.dump(train_data, f, indent=4)

# Save the test dataset to vqa_test.json
with open(os.path.join(output_dir,'sgfood_sharegpt_test.json'), 'w') as f:
    json.dump(test_data, f, indent=4)

print("SGFood train and test datasets generated and saved to sgfood_sharegpt_train.json and sgfood_sharegpt_test.json")